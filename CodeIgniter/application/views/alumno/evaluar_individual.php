...
?>
	...
	<h1 class="datosact">Evaluando a <?php foreach ($evaluado->result() as $a) { echo $a->User;} ?></h1>
...
<script>
	$(document).ready(function(){
		...
		document.getElementById("Volver").addEventListener("click",function(){
			...
			url = "<?php echo base_url() ?>index.php/Alumno/evaluar/<?php echo $this->session->userdata('idreto') ?>"; 
			location.href=url;
		...
		document.getElementById("Enviar").addEventListener("click",function(){
			...
			for(var i=1; i < 5; i++){
				...
				$.post({url: "<?php echo base_url(); ?>index.php/Alumno/agregar_evaluacion",
				datatype:"json",
				data:{
					'ID_Competencia':select1[0],
					'Nota':select1[1],
					'ID_User':<?php foreach ($evaluado->result() as $a) { echo $a->ID_Usuario;} ?>
				}});
			}
			var select1 = $("#select_competencia"+9).val().split("-");
				$.post({url: "<?php echo base_url(); ?>index.php/Alumno/agregar_evaluacion",
				datatype:"json",
				data:{
					'ID_Competencia':select1[0],
