...
<script>
	...
	$(document).ready(function(){
		...
		$("#select_curso").change(function(){
			...
			//SE PONE LAS TRAMPAS "CICLO", "MODULO", "TIPO" Y "USUARIO" A FALSE PARA QUE TENGAS QUE VOLVER A SELECCIONARLAS.
			trampaCiclo = false;
			trampaModulo = false;
			trampaTipo = false;
			trampaUsuario = false;
		...
		$("#select_tipo").change(function(){
	    	...
	    	$('#select_usuario2').append($('<option>Selecciona Usuario</option>'));
		...
		$("#select_usuario2").change(function(){
		...
		$("#botonMostrar").click(function(){
    		...
    		//SI SE HAN SELECCIONADO TODOS LOS PARÁMETROS.
		        ...
		        datatype:"json",
	        	data:{'ID_Usuario':$("#select_usuario2").val()},
		        success: function(devuelto){
		        $('.tablaoculta').css("display","block");
		        	...
		        	for (var i = 0; i < array.length; i++) {
		            	...
		            	$('#tabla_usuarios_modulos_tipo').append($('<tr class="contenido"><td>'+$("#select_curso option:selected").text()+'</td><td>'+$("#select_ciclo option:selected").text()+'</td><td>'+$("#select_modulo option:selected").text()+'</td><td>'+$("#select_tipo option:selected").text()+'</td><td>'+array[i]['User']+'</td><td>'+array[i]['Nombre']+'</td><td>'+array[i]['Apellidos']+'</td></tr>'));
		...
		$("#botonCrear").click(function(){
		    ...
		    datatype:"json",
	        data:{'ID_Usuario':$("#select_usuario2").val(),'ID_Modulo':$("#select_modulo").val()},
		    success: function(devuelto){
