...
<script>
	...
	$(document).ready(function(){
    		...
    		//SI SE HAN SELECCIONADO TODOS LOS PARÁMETROS.
		        ...
		        $('.tablaoculta').css("display","block");
		        	...
		        	for (var i = 0; i < array.length; i++) {
		        		...
		        		var url_borrar = "Usuario_Modulo/borrar_usuario_modulo/"+array[i]['ID_Usuario_Modulo'];
		        		var url_editar = "Usuario_Modulo/editar_usuario_modulo/"+array[i]['ID_Usuario_Modulo'];


		        		//RELLENA LA TABLA CON LOS USUARIOS ASIGNADOS A MÓDULOS.
		            	$('#tabla_usuarios_modulos').append($('<tr class="contenido"><td id="datosCurso">'+$("#select_curso option:selected").text()+'</td><td id="datosCentro">'+$("#select_centro option:selected").text()+'</td><td id="datosCiclo">'+$("#select_ciclo option:selected").text()+'</td><td id="datosModulo">'+$("#select_modulo option:selected").text()+'</td><td id="datosTipoUsuario">'+array[i]['DESC_TUsuario']+'</td><td id="datosUsuario">'+array[i]['User']+'</td><td>'+array[i]['Nombre']+'</td><td>'+array[i]['Apellidos']+'</td><td><a href='+url_editar+'><input type="button" value="Editar"></input></a><a href='+url_borrar+'><input type="button" value="Borrar"></input></a></td></tr>'));
		...
		$("#botonAñadirNuevo").click(function(){
			...
			window.location.href = "Usuario_Modulo/nuevo_usuario_modulo";
