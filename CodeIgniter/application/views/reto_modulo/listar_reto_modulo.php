...
<script>
	...
	$(document).ready(function(){
		...
		$("#select_curso").change(function(){
			...
			//SE PONE LAS TRAMPAS "CENTRO", "CICLO" Y "MODULO" A FALSE PARA QUE TENGAS QUE VOLVER A SELECCIONARLAS.
			trampaCentro = false;
			trampaCiclo = false;
			trampaModulo = false;
    		...
    		//SI SE HAN SELECCIONADO TODOS LOS PARÁMETROS.
		        ...
		        $('.tablaoculta').css("display","block");
		        	...
		        	for (var i = 0; i < array.length; i++) {
		        		...
		        		var url_borrar = "Reto_Modulo/borrar_reto_modulo/"+array[i]['ID_Reto_modulo'];
		        		var url_editar = "Reto_Modulo/editar_reto_modulo/"+array[i]['ID_Reto_modulo'];


		        		//RELLENA LA TABLA CON LOS RETOS ASIGNADOS A MÓDULOS.
		            	$('#tabla_retos_modulos').append($('<tr class="contenido"><td id="datosCurso">'+$("#select_curso option:selected").text()+'</td><td id="datosCentro">'+$("#select_centro option:selected").text()+'</td><td id="datosCiclo">'+$("#select_ciclo option:selected").text()+'</td><td id="datosModulo">'+$("#select_modulo option:selected").text()+'</td><td id="datosTipoUsuario">'+array[i]['COD_Reto']+'</td><td id="datosUsuario">'+array[i]['DESC_Reto']+'</td><td><a href='+url_editar+'><input type="button" value="Editar"></input></a><a href='+url_borrar+'><input type="button" value="Borrar"></input></a></td></tr>'));
		...
		$("#botonAñadirNuevo").click(function(){
			...
			window.location.href = "Reto_Modulo/nuevo_reto_modulo";
