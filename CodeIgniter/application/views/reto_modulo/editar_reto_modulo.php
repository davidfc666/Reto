			...
			if ($reto){
				...
				foreach ($reto->result() as $ret) {
					...
					<tr>
						...
						<td value='$ret->ID_Centro'>$ret->DESC_Centro</td>
...
<script>
	...
	$(document).ready(function(){
		...
		$("#select_curso").change(function(){
			...
			//SE PONE LAS TRAMPAS "CICLO", "CENTRO" Y "MODULO" A FALSE PARA QUE TENGAS QUE VOLVER A SELECCIONARLAS.
			trampaCentro = false;
			trampaCiclo = false;
			trampaModulo = false;
		...
		$("#botonMostrar").click(function(){
    		...
    		//SI SE HAN SELECCIONADO TODOS LOS PARÁMETROS.
		        ...
		        $('#tabla_retos_modulos').append($('<tr class="contenido"><td id="datosCurso">'+$("#select_curso option:selected").text()+'</td><td id="datosCentro">'+$("#select_centro option:selected").text()+'</td><td id="datosCiclo">'+$("#select_ciclo option:selected").text()+'</td><td id="datosModulo">'+$("#select_modulo option:selected").text()+'</td><td id="datosReto">'+"<?php echo $reto->result()[0]->COD_Reto; ?>"+'</td><td id="datosDescripcionReto">'+"<?php echo $reto->result()[0]->DESC_Reto; ?>"+'</td></tr>'));
		...
		$("#botonVolver").click(function(){
    		...
    		window.location.href = "../../Reto_Modulo";
	    });
		...
		$("#botonEditar").click(function(){
		    ...
		    $.post({url: "<?php echo base_url(); ?>index.php/Reto_Modulo/guardar_cambios_reto_modulo/<?php echo $reto->result()[0]->ID_Reto_modulo; ?>",
		    datatype:"json",
