...
<script>
	...
	$(document).ready(function(){
		...
		$("#select_curso").change(function(){
			...
			//SE PONE LAS TRAMPAS "CENTRO", "CICLO", "MODULO" Y "RETO" A FALSE PARA QUE TENGAS QUE VOLVER A SELECCIONARLAS.
			trampaCentro = false;
			trampaCiclo = false;
			trampaModulo = false;
			trampaReto = false;
    		...
    		//SI SE HAN SELECCIONADO TODOS LOS PARÁMETROS.
		        ...
		        $('.tablaoculta').css("display","block");
		        	...
		        	for (var i = 0; i < array.length; i++) {
		            	...
		            	$('#tabla_retos_modulos').append($('<tr class="contenido"><td>'+$("#select_curso option:selected").text()+'</td><td>'+$("#select_centro option:selected").text()+'</td><td>'+$("#select_ciclo option:selected").text()+'</td><td>'+$("#select_modulo option:selected").text()+'</td><td>'+$("#select_reto option:selected").text()+'</td><td>'+array[i]['DESC_Reto']+'</td></tr>'));
		...
		$("#botonCrear").click(function(){
		    ...
		    $.post({url: "<?php echo base_url(); ?>index.php/Reto_Modulo/nuevo_reto_modulo_crear",
		    datatype:"json",
